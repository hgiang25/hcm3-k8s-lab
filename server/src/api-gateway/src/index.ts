...
const PROFILE_API_PREFIX = SERVER_CONFIG.PROFILE_SERVICE.API.PREFIX;
const PROFILE_API_URL =
  SERVER_CONFIG.PROFILE_SERVICE.SERVER_ORIGIN +
  ':' +
  SERVER_CONFIG.PROFILE_SERVICE.PORT;
...
const getSessionInfo = async (authHeader?: string) => {
  ...
  if (!!authHeader) {
    ...
    try {
      ...
      // not a valid JWT, fallback to redis
    }
  } else {
  ...
  if (nodeRedisClient) {
    ...
    if (!exists) {
      ...
      let newSessionData: ISessionData = {
        userId: 'USR_' + randomUUID(),
        persona,
      };
      await nodeRedisClient.set(sessionId, JSON.stringify(newSessionData)); //random new userId
...
app.use(async (req: any, res: any, next: any) => {
  ...
  if (sessionInfo?.sessionData && sessionInfo?.sessionId) {
    ...
    // Route guard for Admin endpoints
    if (req.path.includes('/getOrderStats') && req.role !== 'ADMIN') {
